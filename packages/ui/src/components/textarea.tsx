...
function Textarea({ className, dense = false, ...props }: TextareaProps) {
  return (
    <textarea
      ...
      className={cn(
        "min-h-28 w-full resize-y rounded-lg border border-input bg-surface px-3 text-base leading-relaxed text-foreground transition-[background-color,border-color,box-shadow] duration-(--duration-normal) ease-(--ease-emphasized) outline-none placeholder:text-muted-foreground focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/35 disabled:cursor-not-allowed disabled:bg-muted disabled:opacity-60 aria-invalid:border-danger aria-invalid:ring-3 aria-invalid:ring-danger/20 sm:text-sm",
