...
import type { FixtureEncounterEditGuardPort } from "../domain/ports/fixture-editing.ports.ts";
...
export class CreateScheduleChangeRequestUseCase {
  ...
  async execute(
    ...
    return this.deps.mutationLock.runExclusive(input.encounterId, async () => {
      ...
      const active = await this.deps.requests.findActiveByEncounter(
        input.organizationId,
        input.encounterId,
      );
      if (active) {
        return err(
          new ActiveScheduleChangeRequestExists({
            ...
            message: "This Encounter already has an active schedule change request",
            encounterId: input.encounterId,
      ...
      if (appliedReschedules >= competitionRules.maxReschedulesPerTeam) {
        return err(
          new RescheduleLimitReached({
            ...
            message: "The requesting Team has reached its reschedule limit",
            teamId: input.requestingTeamId,
      ...
      if (
        !(await this.deps.editGuard.canEdit({
          organizationId: input.organizationId,
          competitionId: input.competitionId,
          encounterId: input.encounterId,
        }))
        ...
        return err(
          new EncounterNotEditableForScheduleChange({
            ...
            message:
              "An Encounter with a protected slot, selection, or approved result cannot be rescheduled",
            encounterId: input.encounterId,
          }),
        );
      }
