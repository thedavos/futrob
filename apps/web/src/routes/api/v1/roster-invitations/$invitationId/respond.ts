...
export const Route = createFileRoute("/api/v1/roster-invitations/$invitationId/respond")({
  server: {
    handlers: {
            ...
            next: async ({ client }) => {
              ...
              return jsonResponse(
                respondToRosterInvitationResponseSchema.parse(
                  await client.teams.respondToRosterInvitation(params.invitationId, parsed.data),
                ),
              );
            },
          });
