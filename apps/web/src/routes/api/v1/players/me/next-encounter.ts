...
import { getMyNextEncounterResponseSchema } from "@futrob/api-contracts";
import {
...
export const Route = createFileRoute("/api/v1/players/me/next-encounter")({
  server: {
    handlers: {
      GET: async ({ request }) => {
        try {
          ...
          return jsonResponse(
            getMyNextEncounterResponseSchema.parse(await client.teams.getMyNextEncounter()),
          );
        } catch (error) {
