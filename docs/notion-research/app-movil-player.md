...
**Web parity target.** Web already ships the three onboarding paths, atomic finish, EA club association without sending crest bytes, workspace switcher, `EffectiveAccess` gating, player home, matches, statistics, and roster inbox. Mobile should reuse those SDK methods and copy presentation policy, not reimplement use cases. Product HTTP from the device goes to the web BFF (`EXPO_PUBLIC_FUTROB_API_BASE_URL` default `http://localhost:3000`). A user session token against `apps/api` (`:8787`) is `401 api.unauthorized`.

**Open product forks (not settled by code).** AC-MOB-001 says login resumes the persisted onboarding step. Web production always cold-starts at intention. Notion "Mis partidos" names `/matches`. Web `PlayerMatchesPage` loads `GET /players/me/recent-matches` (ProviderMatch), not official `GET /players/me/matches`. There is no `/api/auth/refresh`. Silent 401 refresh has no server method today.
...
There is no `/api/auth/refresh` and no SDK interceptor. `HttpClient` does not retry 401. Home treats 401 by `clearSession` and routing to login. `account.refreshToken` columns on the auth Drizzle schema are OAuth leftovers. Email and password is the only enabled method. Logout only deletes local keys. It does not call `POST /api/auth/sign-out`. `@futrob/logger` is a mobile dependency and is unused. No Sentry in mobile. Web and API Sentry modules also lack an `Authorization` or `Cookie` scrubber. Auth copy is hardcoded Spanish. Auth screens log nothing today, so token leakage in logs is an absence, not a proven redaction policy.
...
- Silent refresh on 401. Missing as a product method. No refresh route. Home clears the session. `GET /api/auth/get-session` may slide `expiresAt` on the same session token. It does not mint a new bearer.
...
| integration | `apps/mobile/src/modules/api/futrob-client.refresh.test.ts` | Given a first product GET 401, when the client calls `GET /api/auth/get-session` with the stored Bearer, then a still-valid session retries the original GET with the same token and a missing session clears SecureStore and shows login. |
...
SecureStore ya guarda el bearer. No hay ruta de refresh. Falta `sign-out` remoto, política ante 401 y pruebas de que el token no sale en logs.
...
- OpenAPI lags the SDK. Missing `operationId`s include `listMine`, `resolvePostAuthDestination`, create org, org-level invitations, `/players/me/teams`, `/players/me/active-team`, `/players/me/game-profile`. Typed alignment is contracts plus SDK, not the YAML alone.
...
`getFutrobClient()` ya envuelve el SDK. OpenAPI va por detrás del SDK. Falta una capa fina de onboarding y player sin copiar reglas de dominio.
...
- Primitives meet 44 dp. Login and signup inline text links ("Crear una cuenta", "Iniciar sesión") are not 44 dp targets.
- Theme tokens are `LIGHT_THEME` while `app.json` `userInterfaceStyle` is `"dark"` and splash is `#101214`.
...
| unit | `apps/mobile/app/(auth)/login.test.tsx` | Given the signup link on login, when hit area is measured, then height is at least `theme.controlHeight`. |
...
Button e Input ya usan 44 dp. Los links de auth no. Falta checklist Must, vacíos de producto y alinear tema claro u oscuro.
...
Three paths in contracts: `organization`, `invitation`, `player`. `saveOnboardingProgress` PATCHes `{ path, currentStep }`. Finish is one POST per path. Skip of optional game account or club is client-side (`goTo` next step with a null field on finish). There is no skip HTTP operation. Invitation skip is different. It switches path to `player` at game-account (`onboarding.invitation.continuePlayer`). Review is a `currentStep` on PATCH, not a finish route.

Web cold bootstrap always opens at intention (`resolveOnboardingStep` ignores server step). Resume of a persisted step is `resolvePersistedOnboardingStep` (Storybook or harness). AC-MOB-001 wants login to resume the persisted step if incomplete. That is a product fork against web production.
...
- Login does not resume `currentStep`. Choose AC-MOB-001 resume or web intention before writing that test as a pass.
...
| unit | `apps/mobile/src/modules/onboarding/onboarding-routing.test.ts` | Given invitation skip, when advancing, then path is `player` and the next step is `game-account`. |
| integration | `apps/mobile/src/modules/onboarding/onboarding-gateway.test.ts` | Given incomplete status `{ path: "player", currentStep: "club" }` and the AC-MOB-001 resume rule, when login resume runs, then the club step is shown, not home. |
...
SDK y API ya orquestan los 3 caminos. Móvil no guarda progreso ni finish. Skip de invitación en web cambia a path player. Resume vs intention sigue abierto.
...
SDK throws `FutrobApiError` with `code` and `messageKey`. Web `finalizationError` maps those codes to i18n keys and never shows provider messages (`provider.secret_message` → `errors.onboarding.completePlayer`). API 409 `identity.onboarding_path_conflict` is not in `catalogs.ts` and is not special-cased in `finalizationError`. It would fall through to `errors.onboarding.finish` or `errors.onboarding.completePlayer`.
...
- Need the same map as web (copy the table, do not import web presentation). Add a key for `identity.onboarding_path_conflict` instead of a generic finish.
...
| unit | `apps/mobile/src/modules/onboarding/finalization-errors.test.ts` | Given 409 `identity.onboarding_path_conflict`, when mapping, then the key is `errors.identity.onboarding_path_conflict`, not a generic finish. |
...
Finish atómico y códigos tipados viven en API y SDK. Móvil no mapea `FutrobApiError`. Falta key para `identity.onboarding_path_conflict`.
...
The Notion criterion names `/players/me/statistics` and `/matches`. The SDK has both official `GET /players/me/matches` (`getMyMatches`, `PlayerMatchContributionDto`) and provider `GET /players/me/recent-matches` (`getMyRecentMatches`). Web `PlayerMatchesPage` uses **recent ProviderMatch** of the selected ExternalClub, with views `all | league | playoff | friendly`. Query `view=recent` is remapped to `all`. Official personal aggregates are `getMyStatistics` and `getMyGameProfile`. Statistics BC only after approved official results. Do not use `client.encounters` as a player feed.

Web pages: `PlayerMatchesPage`, `PlayerStatisticsPage`. Queries in `statistics-queries.ts`.

### Key files and symbols

- `packages/sdk/src/resources/statistics.ts`. `getMyStatistics`, `getMyMatches`, `getMyRecentMatches`, `getMyGameProfile`.
...
- Parity with web means `getMyRecentMatches` first. Official `getMyMatches` is a second screen unless product says otherwise. AC-PLAYER-001 still describes Recientes (7 calendar days) vs Todos. Web no longer implements that pair.

### Tests to write

| Layer | File | Behavior |
| --- | --- | --- |
| integration | `apps/mobile/src/modules/player/matches-loader.test.ts` | Given the adapter used by Mis partidos (web parity), when listing, then the request URL is `/players/me/recent-matches`, not `/encounters` and not `/players/me/matches`. |
...
SDK tiene statistics, matches oficiales y recent-matches. Web pinta ProviderMatch recientes. Móvil no consume ninguno.
...
Web empty states include titles and CTAs (`needs_club` → associate club, `needs_game_account` → `/player/game-accounts`). Match filters are `all | league | playoff | friendly`. Personal space entry is the shell, not an org. Inbox and several accept pages on web are still hardcoded Spanish. Catalog keys are the bilingual contract for mobile.
...
| Layer | File | Behavior |
| --- | --- | --- |
| integration | `apps/mobile/app/(home)/matches.test.tsx` | Given recent-matches `{ kind: "needs_club" }` or an empty list, when the screen renders, then the empty title is shown and a CTA is present. |
...
Paridad web: vacíos `needs_club` / `needs_game_account` y filtros all|league|playoff|friendly. En móvil no existen.
...
Web tiene inbox, claim por token y cupo `teams.roster_full`. Copy del inbox web está hardcodeado en ES; el contrato bilingüe es `catalogs.ts`. Móvil no tiene UI ni resume.
