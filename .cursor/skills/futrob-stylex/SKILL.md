...
- Consume colors through `colors` in `packages/ui/src/styles/tokens.stylex.ts`.
- Breakpoints: `media` in `media.stylex.ts` (`sm` 40rem, `md` 48rem, `lg` 64rem, `maxSm` for touch density).
...
- Import `applyHost`, `colors`, `typography`, `elevation`, `media` from `@futrob/ui` (or `#styles/*` inside the UI package).
