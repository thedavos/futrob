...
- **Return home.** Choose the `Futrob` link. The URL is `/` again.
- **Hero signup.** Choose the hero button `Crear cuenta`. The URL becomes `/signup` again. Return home via `Futrob`.
