...
- **Node 24 is required** (`engines.node >=24`). The Cloud image may also expose an older Node 22 at `/exec-daemon/node` that would otherwise win on `PATH`. `.cursor/cloud-install.sh` installs Node 24 via nvm and prepends it on `PATH`. If `node --version` ever shows 22, run `source ~/.bashrc`.
...
- **Native install scripts**: `workerd`, `esbuild`, and `sharp` fetch binaries in `postinstall`. npm 11.16 still runs them by default. If the Cloud image has npm 12+, the install script writes `dangerously-allow-all-scripts=true` to the VM `~/.npmrc` so `npm ci` is non-interactive. If a fresh `npm ci` warns about blocked install scripts, the Workers dev server will fail to boot until they are rebuilt.
