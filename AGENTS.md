...
- **npm 11 install-script gate**: native deps (`workerd`, `esbuild`, `sharp`) need their install scripts. `package.json` pins an `allowScripts` allowlist; the Cloud install also writes `dangerously-allow-all-scripts=true` to `~/.npmrc` so `npm ci` is non-interactive. If a fresh `npm ci` warns about blocked install scripts, the Workers dev server will fail to boot until they are rebuilt.
