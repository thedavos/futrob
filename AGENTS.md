...
- **Auth serving**: `/api/auth/*` is served by the `apps/auth` worker; web reaches it only through the `AUTH_SERVICE` service binding. `npm run dev` starts auth on `:8788`, and Wrangler connects the separately running local Workers by service name.
