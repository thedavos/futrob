...
- **D1 (auth) local state** lives at `apps/web/.wrangler/state` (shared by the `vp dev` Cloudflare plugin and `apps/auth` via `--persist-to`). If it's reset, re-run `cd apps/auth && npx wrangler d1 migrations apply futrob-app --local` and `cd apps/web && npx wrangler d1 migrations apply futrob-app --local`. `apps/auth` owns the auth schema since ADR-0015 stage 3.
