...
- **Local env files are gitignored and must exist to run the app**: `apps/web/.dev.vars`, `apps/auth/.dev.vars`, and `apps/api/.env`. Web and API `INTERNAL_JOB_SECRET` values **must match**, or the web BFF → API org/onboarding calls fail with 401. Web and auth `BETTER_AUTH_SECRET` values **must match**, or login 200s and BFF/SSR stay unauthenticated. Set `FUTROB_API_BASE_URL=http://localhost:8787/api/v1` and `FUTROB_AUTH_SERVICE_URL=http://localhost:8788` in `apps/web/.dev.vars` so the BFF does not use the production URLs from `wrangler.jsonc`. The Cloud install script copies the examples and aligns those secrets.
- **D1 (auth) local state** lives at `apps/web/.wrangler/state` (shared by the `vp dev` Cloudflare plugin and `apps/auth` via `--persist-to`). If it's reset, run `cd apps/auth && npx wrangler d1 migrations apply futrob-app --local --persist-to ../web/.wrangler/state`, then apply web's lineage from `apps/web`. Without that `--persist-to`, Wrangler migrates an unused `apps/auth/.wrangler/state`. `apps/auth` owns the auth schema since ADR-0015 stage 3.
- **Auth serving**: `/api/auth/*` is served by the `apps/auth` worker; web only proxies it. `npm run dev` starts auth on `:8788`. Override `FUTROB_AUTH_SERVICE_URL` in `.dev.vars` or local sign-up hits the production worker URL in `wrangler.jsonc`.
