...
| FTR-PLAYER-006 | Mostrar en Mis partidos las observaciones recientes (`ProviderMatch`) en las que el jugador aparece, tomadas de los ExternalClub asociados. Sin al menos un `PlayerExternalClubAssociation` no se consulta al proveedor. El identificador de juego no sustituye al club: solo filtra apariciones después de traer esos partidos. Estas observaciones no alimentan `PlayerPersonalStats` ni proyecciones competitivas. |
