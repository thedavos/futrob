...
| **PlayerExternalClubAssociation** | Referencia opcional de un PlayerProfile a un ExternalClub. Un perfil puede tener varias. | Guarda proveedor, id, nombre, plataforma, edición e imageUrl. Es declarativa; no crea Team o Roster ni verifica propiedad. Es **requisito** para consultar partidos recientes del proveedor; sin club no hay fetch. |
...
| Término                                  | Definición canónica                                                                     | Regla o límite                                                                                                                                                                                                                                                                                                              |
| ---------------------------------------- | --------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Team**                                 | Unidad competitiva que ocupa fixtures, tabla y bracket.                                 | En MVP FC Clubs es un club/equipo; no se asume 1v1 individual. Pertenece a una Organization.                                                                                                                                                                                                                                |
| **CompetitionEntry**                     | Inscripción de un Team en una Competition.                                              | Un Team se inscribe como máximo una vez por Competition. Organizer/staff puede añadirlo como aprobado; no existe estado de verificación EA.                                                                                                                                                                                 |
| **Roster / CompetitionRosterMembership** | Pertenencia de un PlayerProfile a un Team inscrito en una Competition.                  | Como máximo un Team por Competition y jugador. Roles de plantilla (jugador, capitán, subcapitán) son distintos de `CompetitionMembership`. Puede amarrar un `PlayerGameAccount` opcional.                                                                                                                                   |
| **ActiveTeamPreference**                 | Preferencia personal del actor sobre qué membresía de plantilla está seleccionada.      | Un solo activo global por actor; no concede elegibilidad ni sustituye reglas de competición.                                                                                                                                                                                                                                |
| **PlayerGameAccount**                    | Identificador externo de EA declarado por un PlayerProfile/Player.                      | Futrob confía en la declaración y no verifica propiedad. El identificador correlaciona partidos oficiales y estadísticas, y filtra apariciones recientes **después** de consultar el ExternalClub seleccionado; no sustituye a `PlayerExternalClubAssociation`. Plataforma y edición contextualizan las consultas a la API. |
| **ExternalClubConnection**               | Asociación operativa entre un Team y un ExternalClub (id, nombre, plataforma, edición). | Localiza partidos del Team. Es distinta de PlayerExternalClubAssociation y no prueba cuenta ni propiedad.                                                                                                                                                                                                                   |
...
| **ProviderMatch**                 | Partido normalizado de cualquier proveedor.                             | No es resultado oficial de competición. Mis partidos lo muestra como observación de proveedor (Recientes / Todos), con tipo (liga / playoff / amistoso) y tarjetas de la aparición cuando alineó con el club seleccionado; si no alineó, «No jugaste». |
...
| **PlayerPersonalStats**                           | Proyección de estadísticas propias del PlayerProfile. | Solo usa partidos oficiales aprobados. No incorpora `ProviderMatch` recientes. Nunca expone payloads raw, disputas o datos administrativos. |
