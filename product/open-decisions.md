...
| DEC-006 | Stack de plataforma           | Web TanStack Start y auth Better Auth en **Cloudflare Workers**; API de producto Hono/Node en **Railway** con **Postgres** y egress EA. D1 para auth/actores y rate limits BFF; R2, Queues y Cron en web. Dominio/application en `packages/<bc>`, composición de producto en `apps/api/src/di`. Ver [arquitectura](/docs/architecture/overview.md). Sin Supabase ni Vercel. |
